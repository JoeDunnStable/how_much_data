...
public:
  ...
  RealType mad2() const {
    ...
    vector<RealType> points{0, 2/(_alpha - 1)};
    RealType result;
    RealType abserr;
    int neval;
    typename IntegrationController<RealType>::TerminationCode t_c;
    int last;
    _ctl_mad2.integrate(f, points, result, abserr, neval, t_c, last);
    return 2 * _sigma * result;
  }
  ...
  complex<RealType> characteristic_function(RealType omega) const{
    ...
    return (_alpha*expint(1+_alpha, arg))*exp(complex<RealType>(0,(_mu-_sigma)*omega));
  }
  ...
  complex<RealType> characteristic_function(complex<RealType> omega) const{
    if (omega == complex<RealType>{0,0}) return complex<RealType>(1,0);
    complex<RealType> i{0.,1.};
    complex<RealType> arg = -i * _sigma * omega;
    if (abs(exp(arg))==0)
      return complex<RealType>(0);
    else
      return (_alpha*expint(1+_alpha, arg))*exp(i * (_mu-_sigma) * omega);
  }
  ...
  complex<RealType> characteristic_function_prime(RealType omega) const{
    if (omega == 0) return complex<RealType>(0,mean());
    complex<RealType> arg(0,-_sigma*omega);
    const complex<RealType> i(0.,1.);
    return i * exp(i*(_mu-_sigma)*omega) * _alpha *
            (_sigma * expint(_alpha,arg) +(_mu-_sigma) * expint(1+_alpha,arg));
  }
  ...
  complex<RealType> characteristic_function_prime(complex<RealType> omega) const{
    if (omega == complex<RealType>{0,0}) return complex<RealType>(0,mean());
    const complex<RealType> i(0.,1.);
    complex<RealType> arg = -i * _sigma * omega;
    if (abs(exp(arg)) == 0)
      return complex<RealType>{0};
...
private:
  ...
  static RealType adj_beta(RealType alpha, RealType x1, RealType x2){
    ...
    vector<RealType> points{x1, x2};
    RealType result;
    RealType abserr;
    int neval;
    typename IntegrationController<RealType>::TerminationCode t_c;
    int last;
    _ctl_beta.integrate(f, points, result, abserr, neval, t_c, last);
    return result;
  }
  ...
  public:
    ...
    void operator() (vector<RealType>& xs) {
      for (auto& x : xs)
        x = operator() (x);
    }
  };
  ...
  RealType _mu;
  RealType _sigma;
  static IntegrationController<RealType>& _ctl_beta;
  static IntegrationController<RealType>& _ctl_mad2;
};
