...
struct student_t_distribution : boost::random::student_t_distribution<RealType> {
  ...
  RealType characteristic_function(RealType omega) const{
    RealType abs_omega = abs(omega);
    RealType a=alpha();
    if (abs_omega < std::numeric_limits<RealType>::epsilon()) return 1;
    else {
      RealType besselk = cyl_bessel_k(a/2.,sqrt(a)*abs_omega);
      return (pow(2,1 - a/2.)*pow(a,a/4.)*pow(abs_omega,a/2.)*besselk)/tgamma(a/2.);
    }
  }

  /// return the derivative of the characteristic function at omega
  RealType characteristic_function_prime(RealType omega) const{
    RealType abs_omega = std::abs(omega);
    if (abs_omega < std::numeric_limits<RealType>::epsilon()) return RealType(0.);
    RealType a = alpha();
    return (pow(a,0.5 + a/4.)*pow(abs_omega,a/2.)*
            (-cyl_bessel_k(-1 + a/2.,sqrt(a)*abs_omega) -
             cyl_bessel_k(1 + a/2.,sqrt(a)*abs_omega))*
            sign(omega))/
           (pow(2,a/2.)*tgamma(a/2.)) +
           (pow(a,1 + a/4.)*pow(abs_omega,-1 + a/2.)*
            cyl_bessel_k(a/2.,sqrt(a)*abs_omega)*
            sign(omega))/
           (pow(2,a/2.)*tgamma(a/2.));
